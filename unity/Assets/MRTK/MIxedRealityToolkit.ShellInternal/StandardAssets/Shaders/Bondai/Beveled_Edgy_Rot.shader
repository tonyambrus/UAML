...
SubShader {
    ...
    //BLOCK_BEGIN Object_To_World_Normal 80

    void Object_To_World_Normal_B80(
        float3 Nrm_Object,
    ...
    //BLOCK_BEGIN Blob_Vertex 71

    void Blob_Vertex_B71(
        float3 Position,
        float3 Normal,
        float3 Tangent,
        float3 Bitangent,
    ...
    //BLOCK_BEGIN Blob_Vertex 72

    void Blob_Vertex_B72(
        float3 Position,
        float3 Normal,
        float3 Tangent,
        float3 Bitangent,
    ...
    //BLOCK_BEGIN Move_Verts 178

    void Move_Verts_B178(
        float Anisotropy,
        float3 P,
        float Radius,
        float Bevel,
    ...
    //BLOCK_BEGIN Object_To_World_Dir 108

    void Object_To_World_Dir_B108(
        float3 Dir_Object,
    ...
    //BLOCK_BEGIN Conditional_Float 141

    void Conditional_Float_B141(
        bool Which,
        float If_True,
        float If_False,
        out float Result    )
    ...
    //BLOCK_BEGIN Object_To_World_Dir 76

    void Object_To_World_Dir_B76(
        float3 Dir_Object,
    ...
    //BLOCK_BEGIN Pick_Radius 117

    void Pick_Radius_B117(
        float Radius,
    ...
    //BLOCK_BEGIN Conditional_Float 84

    void Conditional_Float_B84(
        bool Which,
        float If_True,
        float If_False,
        out float Result    )
    ...
    //BLOCK_BEGIN Greater_Than 85

    void Greater_Than_B85(
        float Left,
        float Right,
    ...
    //BLOCK_BEGIN Remap_Range 153

    void Remap_Range_B153(
        float In_Min,
        float In_Max,
        float Out_Min,
        float Out_Max,
    ...
    VertexOutput vert(VertexInput vertInput)
    {
        ...
        // Tex_Coords
        float2 XY_Q133;
        XY_Q133 = (vertInput.uv0-float2(0.5,0.5))*_Decal_Scale_XY_ + float2(0.5,0.5);
        
        // Object_To_World_Dir
        float3 Tangent_World_Q75;
        float3 Tangent_World_N_Q75;
        float Tangent_Length_Q75;
        Tangent_World_Q75 = (mul((float3x3)unity_ObjectToWorld, float3(1,0,0)));
        Tangent_Length_Q75 = length(Tangent_World_Q75);
        Tangent_World_N_Q75 = Tangent_World_Q75 / Tangent_Length_Q75;

        float3 Normal_World_Q108;
        float3 Normal_World_N_Q108;
        float Normal_Length_Q108;
        Object_To_World_Dir_B108(float3(0,0,1),Normal_World_Q108,Normal_World_N_Q108,Normal_Length_Q108);

        float X_Q126;
        float Y_Q126;
        float Z_Q126;
        To_XYZ_B126(vertInput.vertex.xyz,X_Q126,Y_Q126,Z_Q126);

        // Object_To_World_Dir
        float3 Nrm_World_Q74;
        Nrm_World_Q74 = normalize((mul((float3x3)unity_ObjectToWorld, vertInput.normal)));
        
        float3 Binormal_World_Q76;
        float3 Binormal_World_N_Q76;
        float Binormal_Length_Q76;
        Object_To_World_Dir_B76(float3(0,1,0),Binormal_World_Q76,Binormal_World_N_Q76,Binormal_Length_Q76);

        // Divide
        float Anisotropy_Q77 = Tangent_Length_Q75 / Binormal_Length_Q76;

        float Result_Q117;
        Pick_Radius_B117(_Radius_,_Radius_Top_Left_,_Radius_Top_Right_,_Radius_Bottom_Left_,_Radius_Bottom_Right_,vertInput.vertex.xyz,Result_Q117);

        // Divide
        float Anisotropy_Q101 = Binormal_Length_Q76 / Normal_Length_Q108;

        bool Not_Greater_Than_Q85;
        bool Greater_Than_Q85;
        Greater_Than_B85(Z_Q126,0,Not_Greater_Than_Q85,Greater_Than_Q85);

        // FastsRGBtoLinear
        float4 Linear_Q149;
        Linear_Q149.rgb = saturate(_Left_Color_.rgb*_Left_Color_.rgb);
        Linear_Q149.a=_Left_Color_.a;
        
        // FastsRGBtoLinear
        float4 Linear_Q150;
        Linear_Q150.rgb = saturate(_Right_Color_.rgb*_Right_Color_.rgb);
        Linear_Q150.a=_Right_Color_.a;
        
        // Subtract3
        float3 Difference_Q109 = float3(0,0,0) - Normal_World_N_Q108;

        // From_RGBA
        float4 Out_Color_Q82 = float4(X_Q126, Y_Q126, Z_Q126, 1);

        float Result_Q84;
        Conditional_Float_B84(Greater_Than_Q85,_Bevel_Back_,_Bevel_Front_,Result_Q84);

        float Result_Q142;
        Conditional_Float_B84(Greater_Than_Q85,_Bevel_Back_Stretch_,_Bevel_Front_Stretch_,Result_Q142);

        float3 New_P_Q178;
        float2 New_UV_Q178;
        float Radial_Gradient_Q178;
        float3 Radial_Dir_Q178;
        float3 New_Normal_Q178;
        Move_Verts_B178(Anisotropy_Q77,vertInput.vertex.xyz,Result_Q117,Result_Q84,vertInput.normal,Anisotropy_Q101,Result_Q142,New_P_Q178,New_UV_Q178,Radial_Gradient_Q178,Radial_Dir_Q178,New_Normal_Q178);

        // To_XY
        float X_Q146;
        float Y_Q146;
        X_Q146 = New_UV_Q178.x;
        Y_Q146 = New_UV_Q178.y;

        float3 Pos_World_Q60;
        Object_To_World_Pos_B60(New_P_Q178,Pos_World_Q60);

        float3 Nrm_World_Q80;
        Object_To_World_Normal_B80(New_Normal_Q178,Nrm_World_Q80);

        float4 Blob_Info_Q71;
        #if defined(_BLOB_ENABLE_)
          Blob_Vertex_B71(Pos_World_Q60,Nrm_World_Q74,Tangent_World_N_Q75,Binormal_World_N_Q76,_Blob_Position_,_Blob_Intensity_,_Blob_Near_Size_,_Blob_Far_Size_,_Blob_Near_Distance_,_Blob_Far_Distance_,_Blob_Fade_Length_,_Blob_Pulse_,_Blob_Fade_,Blob_Info_Q71);
        #else
          Blob_Info_Q71 = float4(0,0,0,0);
        #endif

        float4 Blob_Info_Q72;
        #if defined(_BLOB_ENABLE_2_)
          Blob_Vertex_B72(Pos_World_Q60,Nrm_World_Q74,Tangent_World_N_Q75,Binormal_World_N_Q76,_Blob_Position_2_,_Blob_Intensity_,_Blob_Near_Size_2_,_Blob_Far_Size_,_Blob_Near_Distance_,_Blob_Far_Distance_,_Blob_Fade_Length_,_Blob_Pulse_2_,_Blob_Fade_2_,Blob_Info_Q72);
        #else
          Blob_Info_Q72 = float4(0,0,0,0);
        #endif

        float Out_Q153;
        Remap_Range_B153(0,1,0,1,X_Q146,Out_Q153);

        float X_Q134;
        float Y_Q134;
        float Z_Q134;
        To_XYZ_B126(Nrm_World_Q80,X_Q134,Y_Q134,Z_Q134);

        // Mix_Colors
        float4 Color_At_T_Q145 = lerp(Linear_Q149, Linear_Q150,float4( Out_Q153, Out_Q153, Out_Q153, Out_Q153));

        // Negate
        float Minus_F_Q135 = -Z_Q134;
        ...
        float B_Q147;
        float A_Q147;
        R_Q147=Color_At_T_Q145.r; G_Q147=Color_At_T_Q145.g; B_Q147=Color_At_T_Q145.b; A_Q147=Color_At_T_Q145.a;

        // Clamp
        float ClampF_Q136=clamp(0,Minus_F_Q135,1);

        float Result_Q141;
        Conditional_Float_B141(_Decal_Front_Only_,ClampF_Q136,1,Result_Q141);

        // From_XYZW
        float4 Vec4_Q137 = float4(Result_Q141, Radial_Gradient_Q178, G_Q147, B_Q147);

        float3 Position = Pos_World_Q60;
        float3 Normal = Nrm_World_Q80;
        float2 UV = XY_Q133;
        float3 Tangent = Tangent_World_N_Q75;
        float3 Binormal = Difference_Q109;
        float4 Color = Out_Color_Q82;
        float4 Extra1 = Vec4_Q137;
        float4 Extra2 = Blob_Info_Q71;
        float4 Extra3 = Blob_Info_Q72;
    ...
    //BLOCK_BEGIN Blob_Fragment 78

    void Blob_Fragment_B78(
    ...
    //BLOCK_BEGIN FastLinearTosRGB 90

    void FastLinearTosRGB_B90(
        float4 Linear,
        out float4 sRGB    )
    {
    ...
    //BLOCK_BEGIN Fragment_Main 169

    void Fragment_Main_B169(
        float Sun_Intensity,
        float Sun_Theta,
        float Sun_Phi,
        float3 Normal,
    ...
    //BLOCK_BEGIN FingerOcclusion 115

    void FingerOcclusion_B115(
        float Width,
        float DistToCenter,
        float Fuzz,
        float Min_Fuzz,
    ...
    //BLOCK_BEGIN FingerOcclusion 116

    void FingerOcclusion_B116(
        float Width,
        float DistToCenter,
        float Fuzz,
        float Min_Fuzz,
    ...
    //BLOCK_BEGIN Scale_Color 139

    void Scale_Color_B139(
        float4 Color,
        float Scalar,
        out float4 Result    )
    {
    ...
    //BLOCK_BEGIN Fast_Fresnel 170

    void Fast_Fresnel_B170(
        float Front_Reflect,
        float Edge_Reflect,
        float Power,
        float3 Normal,
    ...
    //BLOCK_BEGIN Mapped_Environment 99

    void Mapped_Environment_B99(
    ...
    //BLOCK_BEGIN Sky_Environment 98

    float4 SampleEnv_Bid98(float3 D, float4 S, float4 H, float4 G, float exponent)
    {
    ...
    {
        // main code goes here
        Reflected_Color = SampleEnv_Bid98(Reflected,Sky_Color,Horizon_Color,Ground_Color,Horizon_Power);
    ...
    //BLOCK_BEGIN Min_Segment_Distance 113

    void Min_Segment_Distance_B113(
        float3 P0,
        float3 P1,
        float3 Q0,
        float3 Q1,
    ...
    //BLOCK_BEGIN Finger_Positions 112

    void Finger_Positions_B112(
    ...
    //BLOCK_BEGIN Remap_Range 162

    void Remap_Range_B162(
        float In_Min,
        float In_Max,
        float Out_Min,
        float Out_Max,
    ...
    //BLOCK_BEGIN Conditional_Float 194

    void Conditional_Float_B194(
        bool Which,
        float If_True,
        float If_False,
        out float Result    )
    ...
    half4 frag(VertexOutput fragInput) : SV_Target
    {
        ...
        half4 Blob_Color_Q78;
        #if defined(_BLOB_ENABLE_)
          Blob_Fragment_B78(_Blob_Texture_,fragInput.extra2,fragInput.extra3,Blob_Color_Q78);
        #else
          Blob_Color_Q78 = half4(0,0,0,0);
        #endif

        // Incident3
        float3 Incident_Q87 = normalize(fragInput.posWorld - _WorldSpaceCameraPos);

        // Normalize3
        float3 Normalized_Q86 = normalize(fragInput.normalWorld.xyz);

        // Normalize3
        float3 Normalized_Q119 = normalize(fragInput.tangent.xyz);

        // Color_Texture
        float4 Color_Q131;
        #if defined(_DECAL_ENABLE_)
          Color_Q131 = tex2D(_Decal_,fragInput.uv);
        #else
          Color_Q131 = float4(0,0,0,0);
        #endif
        ...
        float Z_Q138;
        float W_Q138;
        X_Q138=fragInput.extra1.x;
        Y_Q138=fragInput.extra1.y;
        Z_Q138=fragInput.extra1.z;
        W_Q138=fragInput.extra1.w;

        // FastsRGBtoLinear
        float4 Linear_Q91;
        Linear_Q91.rgb = saturate(_Sky_Color_.rgb*_Sky_Color_.rgb);
        Linear_Q91.a=_Sky_Color_.a;
        
        // FastsRGBtoLinear
        float4 Linear_Q92;
        Linear_Q92.rgb = saturate(_Horizon_Color_.rgb*_Horizon_Color_.rgb);
        Linear_Q92.a=_Horizon_Color_.a;
        
        // FastsRGBtoLinear
        float4 Linear_Q93;
        Linear_Q93.rgb = saturate(_Ground_Color_.rgb*_Ground_Color_.rgb);
        Linear_Q93.a=_Ground_Color_.a;
        
        float3 Left_Index_Q112;
        float3 Right_Index_Q112;
        float3 Left_Index_Middle_Q112;
        float3 Right_Index_Middle_Q112;
        Finger_Positions_B112(_Left_Index_Pos_,_Right_Index_Pos_,_Left_Index_Middle_Pos_,_Right_Index_Middle_Pos_,Left_Index_Q112,Right_Index_Q112,Left_Index_Middle_Q112,Right_Index_Middle_Q112);

        // FastsRGBtoLinear
        float4 Linear_Q94;
        Linear_Q94.rgb = saturate(_Albedo_.rgb*_Albedo_.rgb);
        Linear_Q94.a=_Albedo_.a;
        
        // Normalize3
        float3 Normalized_Q155 = normalize(fragInput.binormal.xyz);

        // Incident3
        float3 Incident_Q118 = normalize(fragInput.posWorld - _WorldSpaceCameraPos);

        // One_Minus
        float One_Minus_F_Q172 = 1.0 - Y_Q138;

        float3 New_Normal_Q127;
        Bulge_B127(_Bulge_Enabled_,Normalized_Q86,Normalized_Q119,_Bulge_Height_,fragInput.vertexColor,_Bulge_Radius_,fragInput.binormal.xyz,New_Normal_Q127);

        float Result_Q125;
        SSS_B125(fragInput.binormal.xyz,New_Normal_Q127,Incident_Q87,Result_Q125);

        float4 Result_Q139;
        Scale_Color_B139(Color_Q131,X_Q138,Result_Q139);

        float Transmit_Q170;
        float Reflect_Q170;
        Fast_Fresnel_B170(_Front_Reflect_,_Edge_Reflect_,_Power_,New_Normal_Q127,Incident_Q87,Transmit_Q170,Reflect_Q170);

        // Multiply
        float Product_Q173 = Y_Q138 * Y_Q138;

        float3 NearP_Q113;
        float3 NearQ_Q113;
        float Distance_Q113;
        Min_Segment_Distance_B113(Left_Index_Q112,Left_Index_Middle_Q112,fragInput.posWorld,_WorldSpaceCameraPos,NearP_Q113,NearQ_Q113,Distance_Q113);

        float3 NearP_Q111;
        float3 NearQ_Q111;
        float Distance_Q111;
        Min_Segment_Distance_B113(Right_Index_Q112,Right_Index_Middle_Q112,fragInput.posWorld,_WorldSpaceCameraPos,NearP_Q111,NearQ_Q111,Distance_Q111);

        // Reflect
        float3 Reflected_Q95 = reflect(Incident_Q87, New_Normal_Q127);

        // Multiply_Colors
        float4 Product_Q151 = Linear_Q94 * float4(1,1,1,1);

        // DotProduct3
        float Dot_Q120 = dot(Incident_Q118, Normalized_Q119);

        float Result_Q194;
        Conditional_Float_B194(_Rim_Spread_Texture_,One_Minus_F_Q172,0.5,Result_Q194);

        // Max
        float MaxAB_Q171=max(Reflect_Q170,Product_Q173);

        float NotInShadow_Q115;
        #if defined(_OCCLUSION_ENABLED_)
          FingerOcclusion_B115(_Width_,Distance_Q113,_Fuzz_,_Min_Fuzz_,fragInput.posWorld,fragInput.binormal.xyz,NearP_Q113,_Clip_Fade_,NotInShadow_Q115);
        #else
          NotInShadow_Q115 = 1;
        #endif

        float NotInShadow_Q116;
        #if defined(_OCCLUSION_ENABLED_)
          FingerOcclusion_B116(_Width_,Distance_Q111,_Fuzz_,_Min_Fuzz_,fragInput.posWorld,fragInput.binormal.xyz,NearP_Q111,_Clip_Fade_,NotInShadow_Q116);
        #else
          NotInShadow_Q116 = 1;
        #endif

        float4 Reflected_Color_Q99;
        float4 Indirect_Diffuse_Q99;
        #if defined(_ENV_ENABLE_)
          Mapped_Environment_B99(_Reflection_Map_,_Indirect_Environment_,Reflected_Q95,Reflected_Color_Q99,Indirect_Diffuse_Q99);
        #else
          Reflected_Color_Q99 = float4(0,0,0,1);
          Indirect_Diffuse_Q99 = float4(0,0,0,1);
        #endif

        half4 Reflected_Color_Q98;
        half4 Indirect_Color_Q98;
        #if defined(_SKY_ENABLED_)
          Sky_Environment_B98(New_Normal_Q127,Reflected_Q95,Linear_Q91,Linear_Q92,Linear_Q93,_Horizon_Power_,Reflected_Color_Q98,Indirect_Color_Q98);
        #else
          Reflected_Color_Q98 = half4(0,0,0,1);
          Indirect_Color_Q98 = half4(0,0,0,1);
        #endif

        float Hue_Q123;
        float Saturation_Q123;
        float Value_Q123;
        float Alpha_Q123;
        float3 HSV_Q123;
        To_HSV_B123(Product_Q151,Hue_Q123,Saturation_Q123,Value_Q123,Alpha_Q123,HSV_Q123);

        float Result_Q158;
        Code_B158(Dot_Q120,Result_Q158);

        // Abs
        float AbsA_Q124 = abs(Result_Q158);

        float4 Result_Q180;
        Rim_Light_B180(Normalized_Q155,Normalized_Q86,Incident_Q118,_Rim_Intensity_,_Rim_Texture_,_Rim_View_Shift_,Result_Q194,Result_Q180);

        // Min
        float MinAB_Q106=min(NotInShadow_Q115,NotInShadow_Q116);

        // Add_Colors
        half4 Sum_Q96 = Reflected_Color_Q99 + Reflected_Color_Q98;

        // Add_Colors
        half4 Sum_Q97 = Indirect_Diffuse_Q99 + Indirect_Color_Q98;

        float Out_Q162;
        Remap_Range_B162(-1,1,0,1,Result_Q158,Out_Q162);

        // Modify
        float Product_Q154;
        Product_Q154 = AbsA_Q124 * _Hue_Shift_;
        //Product_Q154 = sign(AbsA_Q124)*sqrt(abs(AbsA_Q124))*_Hue_Shift_;

        float Hue_Q175;
        float Saturation_Q175;
        float Value_Q175;
        float Alpha_Q175;
        float3 HSV_Q175;
        To_HSV_B123(Result_Q180,Hue_Q175,Saturation_Q175,Value_Q175,Alpha_Q175,HSV_Q175);

        // From_XY
        float2 Vec2_Q160 = float2(Out_Q162,0.5);

        float3 HSV_Out_Q156;
        VaryHSV_B156(HSV_Q123,Product_Q154,_Saturation_Shift_,_Value_Shift_,HSV_Out_Q156);

        float3 HSV_Out_Q174;
        VaryHSV_B156(HSV_Q175,_Rim_Hue_Shift_,_Rim_Saturation_Shift_,_Rim_Value_Shift_,HSV_Out_Q174);

        // Color_Texture
        float4 Color_Q159;
        #if defined(_IRIDESCENCE_ENABLED_)
          Color_Q159 = tex2D(_Iridescence_Texture_,Vec2_Q160);
        #else
          Color_Q159 = float4(0,0,0,0);
        #endif

        float X_Q122;
        float Y_Q122;
        float Z_Q122;
        To_XYZ_B122(HSV_Out_Q156,X_Q122,Y_Q122,Z_Q122);

        float X_Q176;
        float Y_Q176;
        float Z_Q176;
        To_XYZ_B122(HSV_Out_Q174,X_Q176,Y_Q176,Z_Q176);

        // Scale_Color
        float4 Result_Q161 = _Iridescence_Intensity_ * Color_Q159;

        float4 Color_Q121;
        From_HSV_B121(X_Q122,Y_Q122,Z_Q122,0,Color_Q121);

        float4 Color_Q177;
        From_HSV_B121(X_Q176,Y_Q176,Z_Q176,0,Color_Q177);

        // Blend_Over
        float4 Result_Q132 = Result_Q139 + (1.0 - Result_Q139.a) * Color_Q121;

        // Scale_Color
        float4 Result_Q179 = _Rim_Intensity_ * Color_Q177;

        float4 Result_Q169;
        Fragment_Main_B169(_Sun_Intensity_,_Sun_Theta_,_Sun_Phi_,New_Normal_Q127,Result_Q132,MaxAB_Q171,_Shininess_,Incident_Q87,_Horizon_Color_,_Sky_Color_,_Ground_Color_,_Indirect_Diffuse_,_Specular_,_Horizon_Power_,_Reflection_,Sum_Q96,Sum_Q97,_Sharpness_,Result_Q125,_Subsurface_,float4(0,0,0,0),Result_Q179,Result_Q161,Result_Q169);

        float4 Result_Q107;
        Scale_RGB_B107(Result_Q169,MinAB_Q106,Result_Q107);

        float4 sRGB_Q90;
        FastLinearTosRGB_B90(Result_Q107,sRGB_Q90);

        // Blend_Over
        half4 Result_Q79 = Blob_Color_Q78 + (1.0 - Blob_Color_Q78.a) * sRGB_Q90;

        // Set_Alpha
        float4 Result_Q88 = Result_Q79; Result_Q88.a = 1;

        float4 Out_Color = Result_Q88;
