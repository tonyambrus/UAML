...
namespace Microsoft.MixedReality.Toolkit.Utilities.Solvers
{
    ...
    public class HandConstraint : Solver
    {
        ...
        private static readonly int SolverSafeZoneCount = Enum.GetValues(typeof(SolverSafeZone)).Length;
        ...
        [SerializeField]
        [Tooltip("Additional degree offset to apply clockwise from the stated SafeZone.")]
        ...
        protected virtual Vector3 CalculateGoalPosition()
        {
            ...
            {
                float distance;
                Ray ray = CalculateProjectedSafeZoneRay(goalPosition, trackedController, safeZone, safeZoneAngleOffset);
        ...
        private static Ray CalculateProjectedSafeZoneRay(Vector3 origin, IMixedRealityController hand, SolverSafeZone handSafeZone, float angleOffset)
        {
            if (angleOffset == 0)
            {
                return CalculateProjectedSafeZoneRay(origin, hand, handSafeZone);
            }
            ...
            var aSafeZone = (SolverSafeZone)(((int)handSafeZone + intOffset) % SolverSafeZoneCount);
            var bSafeZone = (SolverSafeZone)(((int)handSafeZone + intOffset + 1) % SolverSafeZoneCount);
            var a = CalculateProjectedSafeZoneRay(origin, hand, aSafeZone);
            var b = CalculateProjectedSafeZoneRay(origin, hand, bSafeZone);

            var direction = Vector3.Lerp(-a.direction, -b.direction, fracOffset).normalized;
