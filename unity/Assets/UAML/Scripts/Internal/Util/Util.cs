...
namespace Uaml.UX
{
    ...
    internal static class Util
    {
        ...
        internal delegate bool GetValue<Key, Value>(Key key, out Value value);
        internal static bool TryGetInTypeChain<Key, Value>(Type ownerType, Key key, IEnumerable<string> namespaces, GetValue<Key, Value> func, out Value value)
        {
            value = default;
            foreach (var type in GetTypeChain(ownerType))
            {
                if (func(key, out value))
                {
                    return true;
                }
            }
